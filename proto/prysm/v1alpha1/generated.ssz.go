...
// Hash: c270f2a1479e124828e1eb69f8e08f1fb01ac29cc42091e68d872edf4428d1c8
package eth
...
// MarshalSSZ ssz marshals the BlobsAndCommitments object
func (b *BlobsAndCommitments) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(b)
}

// MarshalSSZTo ssz marshals the BlobsAndCommitments object to a target array
func (b *BlobsAndCommitments) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(8)
	...
	// Field (1) 'KzgCommitments'
	if len(b.KzgCommitments) > 16 {
		err = ssz.ErrListTooBig
		return
	}
	for ii := 0; ii < len(b.KzgCommitments); ii++ {
		if len(b.KzgCommitments[ii]) != 48 {
			err = ssz.ErrBytesLength
			return
		}
...
// UnmarshalSSZ ssz unmarshals the BlobsAndCommitments object
func (b *BlobsAndCommitments) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 8 {
		return ssz.ErrSize
...
// SizeSSZ returns the ssz encoded size in bytes for the BlobsAndCommitments object
func (b *BlobsAndCommitments) SizeSSZ() (size int) {
	size = 8
...
// HashTreeRoot ssz hashes the BlobsAndCommitments object
func (b *BlobsAndCommitments) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(b)
}

// HashTreeRootWith ssz hashes the BlobsAndCommitments object with a hasher
func (b *BlobsAndCommitments) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
...
// MarshalSSZ ssz marshals the PolynomialAndCommitment object
func (p *PolynomialAndCommitment) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(p)
}

// MarshalSSZTo ssz marshals the PolynomialAndCommitment object to a target array
func (p *PolynomialAndCommitment) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(52)

	// Offset (0) 'Polynomial'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(p.Polynomial) * 32

	// Field (1) 'KzgCommitment'
	if len(p.KzgCommitment) != 48 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, p.KzgCommitment...)

	// Field (0) 'Polynomial'
	if len(p.Polynomial) > 4096 {
		err = ssz.ErrListTooBig
		return
	}
	for ii := 0; ii < len(p.Polynomial); ii++ {
		if len(p.Polynomial[ii]) != 32 {
			err = ssz.ErrBytesLength
			return
		}
		dst = append(dst, p.Polynomial[ii]...)
	}

	return
}

// UnmarshalSSZ ssz unmarshals the PolynomialAndCommitment object
func (p *PolynomialAndCommitment) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 52 {
		return ssz.ErrSize
	...
	// Field (1) 'KzgCommitment'
	if cap(p.KzgCommitment) == 0 {
		p.KzgCommitment = make([]byte, 0, len(buf[4:52]))
	}
	p.KzgCommitment = append(p.KzgCommitment, buf[4:52]...)
	...
	// Field (0) 'Polynomial'
	{
		...
		p.Polynomial = make([][]byte, num)
		for ii := 0; ii < num; ii++ {
			if cap(p.Polynomial[ii]) == 0 {
				p.Polynomial[ii] = make([]byte, 0, len(buf[ii*32:(ii+1)*32]))
			}
			p.Polynomial[ii] = append(p.Polynomial[ii], buf[ii*32:(ii+1)*32]...)
		}
	}
	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the PolynomialAndCommitment object
func (p *PolynomialAndCommitment) SizeSSZ() (size int) {
	size = 52

	// Field (0) 'Polynomial'
	size += len(p.Polynomial) * 32

	return
}

// HashTreeRoot ssz hashes the PolynomialAndCommitment object
func (p *PolynomialAndCommitment) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(p)
}

// HashTreeRootWith ssz hashes the PolynomialAndCommitment object with a hasher
func (p *PolynomialAndCommitment) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
	...
	// Field (0) 'Polynomial'
	{
		...
		numItems := uint64(len(p.Polynomial))
