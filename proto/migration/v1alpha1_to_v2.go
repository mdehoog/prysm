...
// BeaconStateEip4844ToProto converts a state.BeaconState object to its protobuf equivalent.
func BeaconStateEip4844ToProto(st state.BeaconState) (*ethpbv2.BeaconStateEip4844, error) {
	sourceFork := st.Fork()
	sourceLatestBlockHeader := st.LatestBlockHeader()
	sourceEth1Data := st.Eth1Data()
	sourceEth1DataVotes := st.Eth1DataVotes()
	...
	result := &ethpbv2.BeaconStateEip4844{
		GenesisTime:           st.GenesisTime(),
		GenesisValidatorsRoot: bytesutil.SafeCopyBytes(st.GenesisValidatorsRoot()),
		Slot:                  st.Slot(),
		Fork: &ethpbv1.Fork{
